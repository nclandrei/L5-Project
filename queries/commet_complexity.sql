SELECT issue.key, count(comment.id) AS comment_count, issue.created AS issue_created, issue.wc_summary, issue.wc_description, changelog_history.time_diff, changelog_history_item.field, changelog_history_item.from_string, changelog_history_item.to_string, changelog_history.created AS ch_created
FROM issue
...
INNER JOIN comment ON comment.issue_key = issue.key
